...
### 03. 사전 지식
- 큰 수와 작은 수를 짝지어야 곱이 최소화된다. 검증은 아래와 같다.

  두 수 a, b (a < b)와 두 수 x, y (x < y)가 있을 때,
  ...
  = (a - b)(y - x)
  ```
  `a < b`이므로 `(a - b) < 0`, `x < y`이므로 `(y - x) > 0`이다.
  
  따라서 `(a - b)(y - x) < 0`, 즉 경우2가 항상 더 작다.
